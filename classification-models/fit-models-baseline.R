#---------------------------------------
# This script uses fits classifiers for 
# each problem
#---------------------------------------
...
fit_models <- function(problem, model_type = c("glmnet", "svm"), N = 1, seed = 123){
  ...
  if(paste0(problem, ".csv") %in% list.files(paste0("classification-models/results-baseline/", model_type))){ # Prevent unnecessary re-runs
    return(NA)
  } else{
    ...
    feature_sets <- unique(all_baseline$feature_set)
    ...
    features_train <- all_baseline |> dplyr::filter(train_test == "Train")
    features_test <- all_baseline |> dplyr::filter(train_test == "Test")
    ...
    for(r in seq_len(N)){
      ...
      if(r == 1){ # Canonical train-test split
        ...
        idx <- generate_resample_indx(all_baseline, train_counts, test_counts)
        features_train_r <- all_baseline |> dplyr::filter(id %in% idx$train_ids)
        features_test_r <- all_baseline |> dplyr::filter(id %in% idx$test_ids)
      }
    ...
    write.csv(results, paste0("classification-models/results-baseline/", model_type, "/", problem, ".csv"), row.names = FALSE)
  }
}
