...
# of normalised performance scores for RBF
# SVMs
#-----------------------------------------

#-----------------------------------------
# Author: Trent Henderson, 16 October 2023
#-----------------------------------------

# Load classification results

load("data/outputs_z_rbf.Rda")
load("data/outputs_z_bp_rbf.Rda")
...
outputs_z_rbf <- bind_rows(outputs_z_rbf, outputs_z_bp_rbf)
...
cluster_1 <- z_scores %>%
  filter(method == "fft") %>%
  filter(z > 1) %>%
  filter(problem != "OliveOil") %>% # Quantiles does better so put it in Cluster 2
  pull(problem)
...
p <- clusters %>%
  ...
  geom_tile() +
  geom_rect(aes(xmin = 6.5, xmax = 7.5, ymin = 97.5, ymax = 102.5), fill = NA, colour = "black", size = 1) + # FFT > mean
  geom_rect(aes(xmin = 7.5, xmax = 8.5, ymin = 90.5, ymax = 97.5), fill = NA, colour = "black", size = 1) + # Quantiles > mean
  geom_rect(aes(xmin = 0.5, xmax = 6.5, ymin = 0.5, ymax = 90.5), fill = NA, colour = "black", size = 1) + # Big feature set box
  labs(x = "Feature set",
       y = "Problem",
       fill = "Normalised performance score") +
  scale_fill_gradientn(colours = c("#0571B0", "#92C5DE", "white", "white", "white", "#F4A582", "#CA0020"),
                       breaks = c(-3.5, -3, -2.5, -2, -1, -0.5, 0, 0.5, 1, 2, 2.5, 3),
                       labels = c("< -3.5", "-3", "-2.5", "-2", "-1", "-0.5", "0", "0.5", "1", "2", "2.5", "3"),
                       limits = c(-3.5, 3)) +
  theme_bw() +
...
print(p2)
ggsave("output/z-scored/normalised-performance-score-rbf.pdf", p2, units = "in", height = 16, width = 16)
